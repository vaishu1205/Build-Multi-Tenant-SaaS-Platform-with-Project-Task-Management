import bcrypt from "bcrypt";
import pool from "../config/db.js";

const SUPERADMIN_ID = "00000000-0000-0000-0000-000000000000";
const TENANT_ID = "11111111-1111-1111-1111-111111111111";
const ADMIN_ID = "22222222-2222-2222-2222-222222222222";
const USER1_ID = "33333333-3333-3333-3333-333333333333";
const USER2_ID = "44444444-4444-4444-4444-444444444444";

const PROJECT1_ID = "55555555-5555-5555-5555-555555555555";
const PROJECT2_ID = "66666666-6666-6666-6666-666666666666";

const TASK1_ID = "77777777-7777-7777-7777-777777777777";
const TASK2_ID = "88888888-8888-8888-8888-888888888888";
const TASK3_ID = "99999999-9999-9999-9999-999999999999";
const TASK4_ID = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa";
const TASK5_ID = "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb";

const seed = async () => {
  const superAdminPassword = await bcrypt.hash("Admin@123", 10);
  const adminPassword = await bcrypt.hash("Demo@123", 10);
  const userPassword = await bcrypt.hash("User@123", 10);

  await pool.query(
    `
    INSERT INTO users (id, tenant_id, email, password_hash, full_name, role)
    VALUES ($1, NULL, 'superadmin@system.com', $2, 'Super Admin', 'super_admin')
    ON CONFLICT (tenant_id, email) DO NOTHING
    `,
    [SUPERADMIN_ID, superAdminPassword],
  );

  await pool.query(
    `
    INSERT INTO tenants (id, name, subdomain, status, subscription_plan, max_users, max_projects)
    VALUES ($1, 'Demo Company', 'demo', 'active', 'pro', 25, 15)
    ON CONFLICT (subdomain) DO NOTHING
    `,
    [TENANT_ID],
  );

  await pool.query(
    `
    INSERT INTO users (id, tenant_id, email, password_hash, full_name, role)
    VALUES
      ($1, $2, 'admin@demo.com', $3, 'Demo Admin', 'tenant_admin'),
      ($4, $2, 'user1@demo.com', $5, 'Demo User One', 'user'),
      ($6, $2, 'user2@demo.com', $5, 'Demo User Two', 'user')
    ON CONFLICT (tenant_id, email) DO NOTHING
    `,
    [ADMIN_ID, TENANT_ID, adminPassword, USER1_ID, userPassword, USER2_ID],
  );

  await pool.query(
    `
    INSERT INTO projects (id, tenant_id, name, description, status, created_by)
    VALUES
      ($1, $2, 'Project Alpha', 'First demo project', 'active', $3),
      ($4, $2, 'Project Beta', 'Second demo project', 'active', $3)
    ON CONFLICT (id) DO NOTHING
    `,
    [PROJECT1_ID, TENANT_ID, ADMIN_ID, PROJECT2_ID],
  );

  await pool.query(
    `
    INSERT INTO tasks (
      id, project_id, tenant_id, title, description,
      status, priority, assigned_to, due_date
    )
    VALUES
      ($1, $2, $3, 'Design homepage mockup', 'Create high-fidelity design for homepage', 'todo', 'high', $4, '2024-07-15'),
      ($5, $2, $3, 'Setup backend API', 'Initialize Node.js backend with Express', 'in_progress', 'high', $6, '2024-07-10'),
      ($7, $8, $3, 'Database schema design', 'Design PostgreSQL database schema', 'completed', 'medium', $4, '2024-07-05'),
      ($9, $8, $3, 'Write API documentation', 'Document all API endpoints', 'todo', 'low', NULL, '2024-07-20'),
      ($10, $8, $3, 'Deploy to production', 'Deploy application to cloud', 'todo', 'high', NULL, '2024-07-25')
    ON CONFLICT (id) DO NOTHING
    `,
    [
      TASK1_ID,
      PROJECT1_ID,
      TENANT_ID,
      USER1_ID,
      TASK2_ID,
      USER2_ID,
      TASK3_ID,
      PROJECT2_ID,
      TASK4_ID,
      TASK5_ID,
    ],
  );

  console.log(" DATABASE SEEDED SUCCESSFULLY");
  console.log("   - Super Admin: superadmin@system.com / Admin@123");
  console.log("   - Tenant Admin: admin@demo.com / Demo@123");
  console.log("   - User 1: user1@demo.com / User@123");
  console.log("   - User 2: user2@demo.com / User@123");
};

export default seed;
